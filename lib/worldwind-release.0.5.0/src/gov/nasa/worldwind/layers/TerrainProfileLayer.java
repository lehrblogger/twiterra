 ...
 * <p>
 * Usage: do setEventSource(wwd) to have the graph activated and updated with position changes See public
 * properties for options: keepProportions, follow, unit, start and end latlon...
 * </p>
 * @author Patrick Murris
 * @version $Id: TerrainProfileLayer.java 5176 2008-04-25 21:31:06Z patrickmurris $ 
 */
public class TerrainProfileLayer extends AbstractLayer implements PositionListener, SelectListener
{
     ...
     * <p>Note that the graph can be both minimized and maximized at the same time.
     * The minimized state will take precedence and the graph will display as an icon.
     * When 'un-minimized' it will display as maximized.</p>
     *
    ...
    private void drawGraph(DrawContext dc, Dimension dimension)
    {
        ...
        if (this.showEyePosition && this.follow.equals(FOLLOW_OBJECT) && this.objectPosition != null)
    ...
    public void computeProfile(DrawContext dc)
    {
        ...
        try
        {
            ...
            if (view.getCenterPosition() != null)
                groundPos = new Position(view.getCenterPosition().getLatLon(), 0);
            if (this.follow.equals(FOLLOW_CURSOR))
